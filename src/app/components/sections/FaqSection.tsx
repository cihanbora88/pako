...
const faqItems: FaqItem[] = [
    {
        ...
        answer:
            'Pako sürücüleri yerel restoran ve işletmelerden alınan siparişleri bisikletle teslim eder. Patronsuz, kolektif bir çalışma modeliyle çalışırlar.',
    },
    {
        question: "Pako'nun diğer teslimat platformlarından farkı nedir?",
        answer:
            'Pako kar amacı gütmeyen, kuryeler tarafından yönetilen bir kooperatiftir. Komisyon oranları düşüktür ve tüm kararlar kolektif olarak alınır.',
    },
    {
    ...
    },
    {
        ...
        answer:
            'Evet. Pako sürücüleri profesyonel ekipmanlara sahiptir ve topluluk tarafından denetlenen güvenlik standartlarına uyar.',
    },
];
...
function AccordionItem({
    ...
    return (
        <div
            className={`w-full rounded-lg border-2 border-[var(--color-border)] transition-colors ${isOpen ? 'bg-white dark:bg-[var(--color-gray-800)]' : 'bg-[var(--color-gray-100)] dark:bg-[var(--color-gray-800)]'
                }`}
        >
            <button
                onClick={onToggle}
            ...
            >
                ...
                <svg
                    className={`size-5 shrink-0 text-[var(--color-primary)] transition-transform dark:text-[var(--color-secondary)] ${isOpen ? 'rotate-180' : ''
                        }`}
                    fill="none"
                    viewBox="0 0 20 20"
                >
